...
def update_optim_param_groups(net, optim, qcfg):
    ...
    if len(pg2del) > 0:
        ...
        print(#logger.info(
            f"After deleteing, optimizer now has {len(optim.param_groups)}")
    print(#logger.info(
    ...
    if cv_group_a["params"]:
        ...
        print(#logger.info(
            f"New clipval_activation group added to optimizer: {str_a}")
    if cv_group_w["params"]:
        ...
        print(#logger.info(
            f"New clipval_weight group added to optimizer: {str_w}")

    return optim


def make_quant_module(module, curr_full_name, qcfg, qlast, verbose=False):
    ...
    # get nbits for head
    if qlast:
        if "head" in curr_full_name:
            nbits_a = qcfg.get("nbits_a_head", 32)
            nbits_w = qcfg.get("nbits_w_head", 32)
...
def q_any_net_5(model: nn.Module, qcfg: dict, qlast: bool, verbose: bool = False):
    ...
    Args:
        ...
        qlast (bool): quantize last layer
    ...
    for name, module in pbar:
        ...
        new_module = make_quant_module(module, name, qcfg, qlast)
...
def qmodel_prep(
    ...
    **kwargs,
    ...
    if qcfg["qlayer_name_pattern"] != []:
        ...
        print(#logger.info(
            f"Layers that will not be quantized: {qskip_layer_name}"
        )
    ...
    qlast = kwargs.get("qlast", False)
    if qlast:
        qcfg["qskip_layer_name"] = [layer for layer in qcfg["qskip_layer_name"] if "head" not in layer]
    ...
    print(#logger.info(
        f"Will use single-sided Conv for: \n {qcfg['qsinglesided_name']}\n"
    )

    model = q_any_net_5(model, qcfg, qlast, verbose)
