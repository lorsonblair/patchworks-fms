...
def qmodel_calib(
    ...
    with torch.no_grad():
        ...
        if qcfg["isRNNmodel"]:
            ...
            for i in range(0, Nbatch):
                ...
                print(#logger.info(
                    f"Qmodel calibration (clip_val analysis) in progress: {i+1}/{Nbatch}"
                )
        ...
        for k, v in tempmodel.state_dict().items():
            ...
            cv_sum_dict["value"].append(v.item())
        print(#logger.info(
            f"\nObserved clipvals: ('*' if it's a vector) \n{ pd.DataFrame(cv_sum_dict) }\n"
        )
    ...
    if qcfg["world_size"] > 1 and qcfg["wasDDPmodel"]:
        ...
        print("This model does not need all_reduce")
