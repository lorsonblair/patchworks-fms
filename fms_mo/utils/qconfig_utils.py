...
def qconfig_init(recipe: str = None, args: Any = None, use_mx: bool = False) -> dict:
    ...
    ## lorson's edits
    # to quantize linear head 
    qcfg["nbits_a_head"] = 32
    qcfg["nbits_w_head"] = 32
...
def check_config(config: dict, model_dtype: torch.dtype = None) -> None:
    ...
    ## lorson's edits
    # to check config values for quantized head
    nbits_a_head = config.get("nbits_a_head", 32)
    # Check if integer was given as float (1.0 when it should be 1)
    if isinstance(nbits_a_head, float) and nbits_a_head.is_integer():
        config["nbits_a_head"] = int(nbits_a_head)
        nbits_a_head = int(nbits_a_head)
    if nbits_a_head not in num_bits_settings:
        raise ValueError(
            f"nbits_a_head = {nbits_a_head} is not a supported quantization setting.  "
            f"Should be set one of the following: {num_bits_settings}"
        )

    nbits_w_head = config.get("nbits_w_head", 32)
    # Check if integer was given as float (1.0 when it should be 1)
    if isinstance(nbits_w_head, float) and nbits_w_head.is_integer():
        config["nbits_w_head"] = int(nbits_w_head)
        nbits_w_head = int(nbits_w_head)
    if nbits_w_head not in num_bits_settings:
        raise ValueError(
            f"nbits_w_head = {nbits_w_head} is not a supported quantization setting.  "
            f"Should be set one of the following: {num_bits_settings}"
        )
    ##
