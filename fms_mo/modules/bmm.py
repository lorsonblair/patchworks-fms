...
class QBmm(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # lorson's edits
        # for bmm regularization
        self.bmm1_reg = qcfg.get("bmm1_reg", False)
        self.bmm2_reg = qcfg.get("bmm2_reg", False)
        ...
        #print(f"calib_counter: {self.calib_counter}")
        self.qcfg = qcfg
        ...
        if self.num_bits_m1 not in [32, 16]:
            self.quantize_m1 = get_activation_quantizer(
                ...
                act_reg=self.bmm1_reg, # lorson's edits
            )
        ...
        if self.num_bits_m2 not in [32, 16]:
            self.quantize_m2 = get_activation_quantizer(
                ...
                extend_act_range=bool(self.extend_act_range),
                use_subnormal=self.fp8_use_subnormal,
                act_reg=self.bmm2_reg, # lorson's edits
            )
            if self.calib_counter > 0:
    ...
    def forward(self, m1, m2):
        ...
        if self.calib_counter > 0:
            ...
            if self.calib_counter == 0:
                ...
                #    None
                #)
                self.quantize_calib_m1 = self.quantize_calib_m2 = (
                    None
                )
