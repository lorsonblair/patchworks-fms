...
class SAWBPlusZeroSTE(torch.autograd.Function):
    ...
    @staticmethod
    def forward(
        ctx, input_tensor, num_bits, dequantize, inplace, objSAWB_clip_val, istraining, wgt_reg,
    ):
        
        ## regularize input using kurtosis
        ## 02 used for weight reg 
        if wgt_reg:
            #print("SAWBPlusZeroSTE - reg")
            zeta=1
            num = input_tensor - input_tensor.mean()
            denum = input_tensor.mul(input_tensor).mean().sqrt() + 1e-8
            kurt = ((num / denum) ** 4).mean()
            input_tensor = input_tensor / (zeta*kurt)
...
class PACT2(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.act_reg = kwargs.get("act_reg", False)
...
class PACT2_STE(torch.autograd.Function):
    ...
    def forward(
        ...
        ## regularize input using kurtosis
        ## used for bmm 2. this will remain False
        if act_reg:
            print("PACT2_STE - reg - bmm")
            num = input_tensor - input_tensor.mean()
            denum = input_tensor.mul(input_tensor).mean().sqrt() + 1e-8
            kurt = ((num / denum) ** 4).mean()
            input_tensor = input_tensor * kurt
...
class PACTplus2STE(torch.autograd.Function):
    ...
    def forward(
        ...
        ## regularize input using kurtosis
        ## 02 used for activation 
        if act_reg:
            #print("PACTplus2STE - reg")
            num = input_tensor - input_tensor.mean()
            denum = input_tensor.mul(input_tensor).mean().sqrt() + 1e-8
            kurt = (((input_tensor - input_tensor.mean()) / denum) ** 4).mean()
            input_tensor = input_tensor * kurt
...
class QminmaxSTE(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, input_tensor, num_bits, _dequantize, inplace, cv, cvn, align_zero, act_reg):
        
        if inplace:
            ctx.mark_dirty(input_tensor)

        ## regularize input using kurtosis
        ## used for bmm2_1. remains False
        if act_reg:
            print("QminmaxSTE - reg - bmm")
            num = input_tensor - input_tensor.mean()
            denum = input_tensor.mul(input_tensor).mean().sqrt() + 1e-8
            kurt = ((num / denum) ** 4).mean()
            input_tensor = input_tensor * kurt
